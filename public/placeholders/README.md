...
| `top-sheet-peel-back.webp` | Home lifestyle (Easier on Your Back) | Final photo (in place): lifting the top sheet off the snaps. Improved version: the four grommets, which had photographed brass, were recolored to the nickel hardware the product ships with and the rest of the frame left untouched, so they match `snap-corner-navy.webp` and the other snap photos. |
...
| `system-diagram-clean.webp` | Home two-part system | Final diagram (in place): the supplied two-part system line drawing, 1593x987 WebP. Carries no text — the heading, copy and callout labels render as live, editable page text — and the artwork's three leader lines leave its right edge at 8.76%, 29.69% and 57.70% of the image height, which is what the Home callouts are pinned to. The supplied canvas stops before the foundation closes, so its front bottom edge and centre leg fall outside the frame. |
